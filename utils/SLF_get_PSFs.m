...
% image flip here is not required by the algorithm, but only related to 
% camera orientation in our setup
    ...
    for p = 1:length(fileName)
        ...
        for v = 1:size(coordinates,1)
            ...
            if normalize_psf
            %%%%% normalize the PSF individually, though not strictly
            %%%%% correct. It alleviates artifacts from the PSF intensity fluctuation
            %%%%% during experimental PSF acquisition
